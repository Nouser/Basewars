...
function ENT:Initialize()
	...
	self.Entity:SetNWEntity("socket11",nil)
	self.Entity:SetNWEntity("socket12",nil)
	self.Entity:SetNWEntity("socket13",nil)
	self.Entity:SetNWEntity("socket14",nil)
	self.Entity:SetNWEntity("socket15",nil)
--	self.scrap = false
...
function ENT:giveMoney()
	...
	if(ValidEntity(ply) && !self.Inactive) then
		if ply:CanAfford(CfgVars["supergeneratorcost"] * .025) then
			ply:AddMoney( CfgVars["supergeneratorcost"] * -.025);
			Notify( ply, 2, 3, "Costed $300 to keep a Super Generator Running." );
		else
			Notify( ply, 4, 3, "Super Generator has Shut Off from Lack of Money!" )
			self.Entity:shutOff()
			timer.Destroy( tostring(self.Entity) .. "fuckafkfags")
			timer.Destroy( tostring(self.Entity) .. "notifyoff")
		end
	elseif (self.Inactive) then
		Notify( ply, 4, 3, "A Super Generator is Inactive, Press 'E' on it to Reactivate it." );
	end
end
...
function ENT:shutOff()
	local ply = self.Owner
	self.Inactive = true
	Notify( ply, 1, 3, "NOTICE: A SUPER GENERATOR HAS GONE INACTIVE" );
	Notify( ply, 1, 3, "PRESS 'E' ON IT TO REACTIVATE IT" );
	self.Entity:SetColor(255,0,0,255)
end
function ENT:notifypl()
	local ply = self.Owner
	Notify( ply, 4, 3, "NOTICE: A SUPER GENERATOR IS ABOUT TO GO INACTIVE" );
	Notify( ply, 4, 3, "PRESS 'E' ON IT TO KEEP IT WORKING" );
	self.Entity:SetColor(255,150,150,255)
end

function ENT:Use(activator,caller)
	if activator:CanAfford(CfgVars["supergeneratorcost"] * .025) then
---	if activator:CanAfford(CfgVars["supergeneratorcost"] * .025) && activator==self.Owner then
		timer.Destroy( tostring(self.Entity) .. "fuckafkfags")
		timer.Create( tostring(self.Entity) .. "fuckafkfags", 1200, 1, function() self:shutOff() end)
		timer.Destroy( tostring(self.Entity) .. "notifyoff")
		timer.Create( tostring(self.Entity) .. "notifyoff", 1080, 1, function() self:notifypl() end)
