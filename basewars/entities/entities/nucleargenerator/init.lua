...
function ENT:Initialize()
	...
	ply:GetTable().maxnucleargenerator=ply:GetTable().maxnucleargenerator + 1
	self.Inactive = false
	self.Powdist=1512
---	self.Powdist=1024	
...
---	self.scrap = false
	...
	self.Payout = {CfgVars["nucleargeneratorcost"], "Nuclear Generator"}	
end

function ENT:giveMoney()
	local ply = self.Owner
	if(IsValid(ply) && !self.Inactive) then
		if ply:CanAfford(CfgVars["nucleargeneratorcost"] * .025) then
			ply:AddMoney( CfgVars["nucleargeneratorcost"] * -.025 );
			Notify( ply, 2, 3, "Costed $550 to keep the Nuclear Generator Running." );
		else
			Notify( ply, 4, 3, "Nuclear Generator has Shut Off from Lack of Money!" )
			self.Entity:shutOff()
			timer.Destroy( tostring(self.Entity) .. "fuckafkfags")
			timer.Destroy( tostring(self.Entity) .. "notifyoff")
		end
	elseif (self.Inactive) then
		Notify( ply, 4, 3, "A Nuclear Generator is Inactive, Press 'E' on it to Reactivate it." );
	end
end
...
function ENT:shutOff()
	local ply = self.Owner
	self.Inactive = true
	Notify( ply, 1, 3, "NOTICE: A NUCLEAR GENERATOR HAS GONE INACTIVE" );
	Notify( ply, 1, 3, "PRESS 'E' ON IT TO REACTIVATE IT" );
	self.Entity:SetColor(Color(255,0,0,255))
end
function ENT:notifypl()
	local ply = self.Owner
	Notify( ply, 4, 3, "NOTICE: A NUCLEAR GENERATOR IS ABOUT TO GO INACTIVE" );
	Notify( ply, 4, 3, "PRESS 'E' ON IT TO KEEP IT WORKING" );
	self.Entity:SetColor(Color(255,150,150,255))
end

function ENT:Use(activator,caller)
	if activator:CanAfford(CfgVars["nucleargeneratorcost"] * .025) then
---	if activator:CanAfford(CfgVars["nucleargeneratorcost"] * .025) && activator==self.Owner then
		timer.Destroy( tostring(self.Entity) .. "fuckafkfags")
		timer.Create( tostring(self.Entity) .. "fuckafkfags", 1200, 1, function() self:shutOff() end)
		timer.Destroy( tostring(self.Entity) .. "notifyoff")
		timer.Create( tostring(self.Entity) .. "notifyoff", 1080, 1,function() self:notifypl() end)
