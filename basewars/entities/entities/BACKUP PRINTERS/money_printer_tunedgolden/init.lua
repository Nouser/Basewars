...
function ENT:Initialize()
	...
	ply:GetTable().maxTunedGolden=ply:GetTable().maxTunedGolden + 1
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetNWInt("power",0)
	self.Payout = {CfgVars["tunedgoldencost"], "Tuned Golden Money Silo"}
...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 17600000, 17900000 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 8800000, 9100000 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 4400000, 4700000 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 2200000, 2500000 )
		else
			amount = math.random( 1900000, 2200000 )		
		end
...
//		moneybag:SetAngles(self.Entity:GetAngles())
		...
		Notify( ply, 0, 3, "Tuned Golden Money Silo created $" .. amount );
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && activator==ply && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["tunedgoldencost"] * .05)) then
		ply:AddMoney( CfgVars["tunedgoldencost"] * -.05 )
		self.NearInact = false
...
function ENT:Reload()
	Notify(self.Owner, 0, 3, "Tuned Golden Money Silo Resupplied!")
