...
function ENT:Initialize()
	...
	ply:GetTable().maxTunedIridium=ply:GetTable().maxTunedIridium + 1
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetNWInt("power",0)
	self.Payout = {CfgVars["tunediridiumcost"], "Tuned Iridium Money Printer"}
...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		Notify( ply, 0, 3, "Tuned Iridium Money Printer created $" .. amount );
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && activator==ply && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["tunediridiumcost"] * .05)) then
		ply:AddMoney( CfgVars["tunediridiumcost"] * -.05 )
		self.NearInact = false
...
function ENT:Reload()
	Notify(self.Owner, 0, 3, "Tuned Iridium Money Printer Resupplied!")
