...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 50625000, 57625000 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 38625000, 45625000 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 25125000, 33625000 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 16312500, 20312500 )
		else
			amount = math.random( 9731250, 10731250 )		
		end
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["novaprintercost"] * .1)) then
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["novaprintercost"] * 2), "Level 1 Nova Money Printer"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["novaprintercost"] * 4), "Level 2 Nova Money Printer"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["novaprintercost"] * 8), "Level 3 Nova Money Printer"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["novaprintercost"] * 16), "Level 4 Nova Money Printer"}
	end
