...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 10538, 15038 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 8769, 9269 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 6385, 7885 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 4692, 5192 )
		else
			amount = math.random( 2346, 2846 )		
		end
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["iridiumprintercost"] * .05)) then
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["iridiumprintercost"] * 2), "Level 1 Iridium Money Printer"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["iridiumprintercost"] * 4), "Level 2 Iridium Money Printer"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["iridiumprintercost"] * 8), "Level 3 Iridium Money Printer"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["iridiumprintercost"] * 16), "Level 4 Iridium Money Printer"}
	end
