...
function ENT:Initialize()
	...
	timer.Create( tostring(self.Entity), 90, 0, function() self:giveMoney() end)
	timer.Create( tostring(self.Entity) .. "fuckafkfags", 1200, 1, function() self:shutOff() end)
	timer.Create( tostring(self.Entity) .. "notifyoff", 1000, 1, function() self:notifypl() end)
...
function ENT:Reload()
	...
	timer.Create( tostring(self.Entity) .. "fuckafkfags", 250, 1, function() self:shutOff() end)
	timer.Destroy( tostring(self.Entity) .. "notifyoff")
	timer.Create( tostring(self.Entity) .. "notifyoff", 200, 1, function() self:notifypl() end)
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetColor(255, 215, 0, 254)
