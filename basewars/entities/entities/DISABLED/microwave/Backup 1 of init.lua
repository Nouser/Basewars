...
function ENT:Initialize()
	...
	self.Owner:GetTable().maxMicrowaves=self.Owner:GetTable().maxMicrowaves + 1
	self.Entity:SetNWInt("power",0)
	self.Payout={CfgVars["microwavecost"],"Microwave"}
end
...
function ENT:Use(activator,caller)
	if self.Entity:IsPowered() then
		...
		timer.Create( tostring(self.Entity) .. "food", 1, 1, function() self:createFood() end)
	end
end
