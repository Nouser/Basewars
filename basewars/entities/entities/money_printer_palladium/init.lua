...
function ENT:Initialize()
	...
	ply:GetTable().maxPalladiumPrinter=ply:GetTable().maxPalladiumPrinter + 1
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetNWInt("power",0)
--	self.Payout = {CfgVars["palladiumprintercost"], "Palladium Money Printer"}
...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 86308, 107308 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 40154, 50154 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 29077, 30077 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 14538, 15538 )
		else
			amount = math.random( 7269, 8269 )		
		end	
		...
		Notify( ply, 0, 3, "Palladium Money Printer created $" .. amount );
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["palladiumprintercost"] * .05)) then
		ply:AddMoney( CfgVars["palladiumprintercost"] * -.05 )
		self.NearInact = false
...
function ENT:Reload()
	Notify(self.Owner, 0, 3, "Palladium Money Printer Resupplied!")
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["palladiumprintercost"] * 2), "Level 1 Palladium Money Printer"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["palladiumprintercost"] * 4), "Level 2 Palladium Money Printer"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["palladiumprintercost"] * 8), "Level 3 Palladium Money Printer"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["palladiumprintercost"] * 16), "Level 4 Palladium Money Printer"}
	end
