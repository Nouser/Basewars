...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 4105714, 5105714 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 2505714, 3105714 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 1502857, 2002857 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 1051429, 1351429 )
		else
			amount = math.random( 875714, 975714 )		
		end
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["moneyfactorycost"] * .05)) then
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["moneyfactorycost"] * 2), "Level 1 Money Factory"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["moneyfactorycost"] * 4), "Level 2 Money Factory"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["moneyfactorycost"] * 8), "Level 3 Money Factory"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["moneyfactorycost"] * 16), "Level 4 Money Factory"}
	end
