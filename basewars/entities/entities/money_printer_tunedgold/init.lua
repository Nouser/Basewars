...
function ENT:Initialize()
	...
	ply:GetTable().maxTunedGold=ply:GetTable().maxTunedGold + 1
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetNWInt("power",0)
	self.Payout = {CfgVars["tunedgoldcost"], "Tuned Gold Money Printer"}
...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 116000, 219000 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 68000, 71100 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 44000, 54000 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 27000, 30000 )
		else
			amount = math.random( 24000, 27000 )		
		end	
		...
		Notify( ply, 0, 3, "Tuned Gold Money Printer created $" .. amount );
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["tunedgoldcost"] * .05)) then
		ply:AddMoney( CfgVars["tunedgoldcost"] * -.05 )
		self.NearInact = false
...
function ENT:Reload()
	Notify(self.Owner, 0, 3, "Tuned Gold Money Printer Resupplied!")
