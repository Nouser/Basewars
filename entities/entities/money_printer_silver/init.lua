...
--	self.Payout = {CfgVars["silverprintercost"], "Silver Money Printer"}
...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 2000, 2600 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 1300, 1500 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 1000, 1100 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 500, 600 )
		else
			amount = math.random( 250, 350 )		
		end		
		...
		Notify( ply, 0, 3, "Silver Money Printer created $" .. amount );
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["silverprintercost"] * .05)) then
		ply:AddMoney( CfgVars["silverprintercost"] * -.05 )
		self.NearInact = false
...
function ENT:Reload()
	Notify(self.Owner, 0, 3, "Silver Money Printer Resupplied!")
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["silverprintercost"] * 2), "Level 1 Silver Money Printer"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["silverprintercost"] * 4), "Level 2 Silver Money Printer"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["silverprintercost"] * 8), "Level 3 Silver Money Printer"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["silverprintercost"] * 16), "Level 4 Silver Money Printer"}
	end
