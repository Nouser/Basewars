...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 20755000, 31755000 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 8755000, 10755000 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 5625000, 6755000 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 3125000, 4625000 )
		else
			amount = math.random( 2562500, 3062500 )		
		end
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["goldensilocost"] * .05)) then
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["goldensilocost"] * 2), "Level 1 Golden Money Silo"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["goldensilocost"] * 4), "Level 2 Golden Money Silo"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["goldensilocost"] * 8), "Level 3 Golden Money Silo"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["goldensilocost"] * 16), "Level 4 Golden Money Silo"}
	end
