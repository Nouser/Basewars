...
function ENT:Initialize()
	...
	self.Entity:SetModel( "models/props/de_train/Processor.mdl" )
...
//	self.Dish2:SetModel( "models/props/de_train/Processor.mdl" )
...
//	self.Dish2:SetMoveType(MOVETYPE_NONE)
	...
	self.Payout = {CfgVars["stablemethlab"],"Stable Meth Lab"}
	...
	if self.drugmaking then
		...
		if(ply:Alive() && !self.Inactive) then
			if (self.Entity:GetNWInt("upgrade")==2) then
				ply:AddMoney( 5000 );
				Notify( ply, 2, 3, "Paid $5000 for Selling High Quality Meth." );
			elseif(self.Entity:GetNWInt("upgrade")==1) then
				ply:AddMoney( 2500 );
				Notify( ply, 2, 3, "Paid $2500 for Selling High Quality Meth." );
			else
				ply:AddMoney( 1250 );
				Notify( ply, 2, 3, "Paid $1250 for Selling High Quality Meth." );
			end
