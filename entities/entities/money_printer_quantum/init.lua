...
function ENT:Initialize()
	...
	self.Entity:SetNWInt("damage",5400)
	self.Entity:SetNWInt("upgrade", 0)
	local ply = self.Owner
	ply:GetTable().maxQuantumFactory=ply:GetTable().maxQuantumFactory + 1
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetNWInt("power",0)
--	self.Payout = {CfgVars["quantumfactorycost"], "Quantum Money Factory"}
...
function ENT:giveMoney()
	...
	if(IsValid(ply) && !self.Inactive && self.Entity:IsPowered()) then
		...
		if (self.Entity:GetNWInt("upgrade")==4) then
			amount = math.random( 45524500, 62524500 )
		elseif (self.Entity:GetNWInt("upgrade")==3) then
			amount = math.random( 26524500, 39524500 )
		elseif (self.Entity:GetNWInt("upgrade")==2) then
			amount = math.random( 18524500, 23524500 )			
		elseif (self.Entity:GetNWInt("upgrade")==1) then
			amount = math.random( 14762250, 16762250 )
		else
			amount = math.random( 11062250, 13762250 )		
		end
...
//		moneybag:SetAngles(self.Entity:GetAngles())
		...
		Notify( ply, 0, 3, "Quantum Money Factory created $" .. amount );
...
function ENT:Use(activator,caller)
	local ply = self.Owner
	if (self.NearInact==true && self.Entity:GetNWBool("sparking")==false && ply:CanAfford(CfgVars["quantumfactorycost"] * .1)) then
...
function ENT:Reload()
	Notify(self.Owner, 0, 3, "Quantum Money Factory Resupplied!")
	...
	self.Inactive = false
	self.NearInact = false
	self.Entity:SetColor(Color(255, 160, 20, 254))	
...
function ENT:Think()
	if self:GetNWInt("upgrade")==1 then
			self.Payout = {(CfgVars["quantumfactorycost"] * 2), "Level 1 Quantum Money Factory"}
	end
	if self:GetNWInt("upgrade")==2 then
			self.Payout = {(CfgVars["quantumfactorycost"] * 4), "Level 2 Quantum Money Factory"}
	end
	if self:GetNWInt("upgrade")==3 then
			self.Payout = {(CfgVars["quantumfactorycost"] * 8), "Level 3 Quantum Money Factory"}			
	end
	if self:GetNWInt("upgrade")==4 then
			self.Payout = {(CfgVars["quantumfactorycost"] * 16), "Level 4 Quantum Money Factory"}
	end
