...
function ENT:Explode()
	// shield slayer
	GAMEMODE:RaidSystemCheckBigbomb(self.Owner)
	...
	if (self.Armed) then
		if (self.LastUsed+0.3>CurTime() && self.Disarming==60) then
			...
			if (self.Disarming<=0) then
				Notify(activator,1,3, "Big Bomb defused!")
				Notify(self.Owner,1,3, "Your Big Bomb has been defused.")
...
function ENT:Armbomb(planter)
	...
	if (self.Entity:CheckPropFaggotry()==false) then
		...
		timer.Create( tostring(self.Entity) .. "checkblast", 119, 1, function() self:PropCheck() end)
