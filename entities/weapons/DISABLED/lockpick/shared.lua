...
function SWEP:PrimaryAttack()
	...
	if (ValidEntity(trace.Entity)) then	
		...
		elseif (trace.HitPos:Distance(self.Owner:GetShootPos()) <= 75 and (trace.Entity:GetClass()=="gunvault") and ValidEntity(trace.Entity)) then
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			self.Owner:FireBullets(bullet) 
			self.Weapon:EmitSound("physics/flesh/flesh_impact_bullet" .. math.random( 3, 5 ) .. ".wav")
			...
			elseif(trace.Entity:GetNWInt("unlockAmount") < 9) then
				...
				trace.Entity:SetNWInt("unlockAmount", 0)
				if SERVER then
					Notify(self.Owner, 4, 3, "Gun Vault Unlocked!")
					trace.Entity:SetUnLocked()
				end
			end
		elseif (trace.HitPos:Distance(self.Owner:GetShootPos()) <= 75 and (trace.Entity:GetClass()=="pillbox") and ValidEntity(trace.Entity)) then
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			self.Owner:FireBullets(bullet) 
			self.Weapon:EmitSound("physics/flesh/flesh_impact_bullet" .. math.random( 3, 5 ) .. ".wav")
			...
			elseif(trace.Entity:GetNWInt("unlockAmount") < 9) then
				...
				trace.Entity:SetNWInt("unlockAmount", 0)
				if SERVER then
					Notify(self.Owner, 4, 3, "Drug Vault Unlocked!")
					trace.Entity:SetUnLocked()
				end
			end		
...
function _R.Entity:IsDoor()
end

function _R.Entity:Keypad()
end

function _R.Entity:gunvault()
end

function _R.Entity:pillbox()
end

function _R.Entity:sent_keypad()
end
