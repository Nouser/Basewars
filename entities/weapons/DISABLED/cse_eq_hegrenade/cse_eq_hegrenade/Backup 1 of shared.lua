...
function SWEP:Think()
	...
	if (self.Owner:GetNetworkedInt("Primed") == 1 and not self.Owner:KeyDown(IN_ATTACK)) then
		if self.Owner:GetNetworkedInt("Throw") < CurTime() then
			...
			timer.Simple(0.35, function()	if not self.Owner then return end self:ThrowGrenade() self.Owner:ViewPunch(Angle(math.Rand(1, 2), math.Rand(0, 0), math.Rand(0, 0))) end)
		end
	end
