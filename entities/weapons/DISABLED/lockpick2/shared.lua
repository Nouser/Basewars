...
function SWEP:PrimaryAttack()
	...
	if ent:IsValid() and table.HasValue({"func_door","prop_door_rotating","func_door_rotating"}, ent:GetClass()) then
		...
		if cha != 1 then
			...
			self.Weapon:EmitSound("play items/ammocrate_open.wav")
			self.Owner:PrintMessage(HUD_PRINTTALK, "You Successfully Broke The Lock And Latch On The Door")
			ent:Fire("Unlock","",0.1)
	...
	if (ValidEntity(trace.Entity)) then	
		if (trace.HitPos:Distance(self.Owner:GetShootPos()) <= 75 and not trace.Entity:GetClass()!="gunvault" && trace.Entity:GetClass()!="pillbox" && trace.Entity:GetClass()!="gunvault" && trace.Entity:GetClass()!="sent_keypad") then
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			self.Owner:FireBullets(bullet) 
			self.Weapon:EmitSound("physics/flesh/flesh_impact_bullet" .. math.random( 3, 5 ) .. ".wav")
			
		elseif (trace.HitPos:Distance(self.Owner:GetShootPos()) <= 75 and (trace.Entity:GetClass()=="gunvault" || trace.Entity:GetClass()=="pillbox" || trace.Entity:GetClass()=="moneyvault") and ValidEntity(trace.Entity)) then
			self.Weapon:SendWeaponAnim(ACT_VM_HITCENTER)
			self.Owner:FireBullets(bullet) 
			self.Weapon:EmitSound("physics/flesh/flesh_impact_bullet" .. math.random( 3, 5 ) .. ".wav")
			...
			elseif(trace.Entity:GetNWInt("unlockAmount") < 9) then
				...
				trace.Entity:SetNWInt("unlockAmount", 0)
				if SERVER then
					Notify(self.Owner, 4, 3, "The Monevault/Pillbox/Gunvault has been unlocked.")
